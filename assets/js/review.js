...
function show(){size();frame.src='/'+page.value+'?lang='+lang.value;}
...
document.getElementById('audit').addEventListener('click',async event=>{
 ...
 for(const p of ['','travel/','photos/','lists/','archive/'])for(const l of ['en','es','ja','zh'])for(const w of [1440,1024,768,390]){
  ...
  await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('Page timed out')),12000);frame.onload=()=>{clearTimeout(timer);resolve();};frame.src='/'+p+'?lang='+l;});
  ...
  if(doc.querySelector('meta[name="robots"]')?.content!=='index,follow,max-image-preview:large')errors.push('public indexing metadata missing');
