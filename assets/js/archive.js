...
async function loadFamily(){
  try{
    const response=await fetch('/assets/genealogy/family.json',{cache:'no-store'});
    ...
    if(data.schema===2){
      ...
      media=Object.fromEntries((data.media||[]).map(m=>{
        ...
        return [id,{id,url:'/assets/genealogy/media/'+filename,title:m[2],kind:m[3],format:filename.split('.').pop()}];
      }));
    }else{
