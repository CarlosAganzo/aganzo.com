import {load,translate as t,onLanguage} from './site.js';
const photos=await load('photos');
const dialog=document.getElementById('photo-dialog');
const img=document.getElementById('viewer-image');
let current=0;
function show(i){current=(i+photos.length)%photos.length;const photo=photos[current];img.src='/v2/assets/images/'+photo.file;img.alt=t(photo.caption);img.width=photo.width;img.height=photo.height;document.getElementById('viewer-caption').textContent=`${String(current+1).padStart(2,'0')} / ${t(photo.caption)}`;}
