...
def run(args):
    ...
    for k,n in objects.items():
        ...
        published[k]={'id':k,'url':'/v2/assets/genealogy/media/'+filename,'title':PurePosixPath(path).name,'format':suffix[1:],'kind':'document' if suffix=='.pdf' or path.startswith('Documentos/') or 'CONSEJOS,' in path else 'photo','sha256':digest,'bytes':len(data)}
    people=[]
